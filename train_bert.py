...
def train(datasets, idx):
    ...
    training_args = TrainingArguments(
        output_dir=f"./bert_finetune_{idx}",
        eval_strategy="steps",
    ...
    # Save the model
    model.save_pretrained(f"./bert_finetuned_{idx}")
    tokenizer.save_pretrained(f"./bert_finetuned_{idx}")


if __name__ == '__main__':
    train_data = [
        'tasks_train_simple_p1.txt',  'tasks_train_simple_p2.txt',
        'tasks_train_simple_p4.txt',  'tasks_train_simple_p8.txt',
        'tasks_train_simple_p16.txt', 'tasks_train_simple_p32.txt',  
        'tasks_train_simple_p64.txt'
    ]
    test_data = [
        'tasks_test_simple_p1.txt',  'tasks_test_simple_p2.txt',   
        'tasks_test_simple_p4.txt',  'tasks_test_simple_p8.txt',  
        'tasks_test_simple_p16.txt', 'tasks_test_simple_p32.txt',  
        'tasks_test_simple_p64.txt' 
    ]
    dir = './data/simple_split/size_variations/'
    for idx, (train_data_file, test_data_file) in enumerate(zip(train_data, test_data)):
        # Load dataset
